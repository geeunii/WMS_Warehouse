...
######################### 요금 관리 ########################
-- 요금 등록
DROP PROCEDURE IF EXISTS sp_InsertFee;
DELIMITER $$
CREATE PROCEDURE sp_InsertFee(
    ...
    OUT out_newFeeID INT
)
BEGIN
    INSERT INTO WarehouseFee (startDate, endDate, price, warehouseID)
    VALUES (p_startDate, p_endDate, p_price, p_warehouseID);

    SET out_newFeeID = LAST_INSERT_ID();
END $$
DELIMITER ;

-- 요금 정보 수정
DROP PROCEDURE IF EXISTS sp_UpdateFee;
DELIMITER $$
...
-- 요금 삭제
DROP PROCEDURE IF EXISTS sp_DeleteFee;
DELIMITER $$
CREATE PROCEDURE sp_DeleteFee(
    IN p_feeID INT
)
BEGIN
    DELETE FROM WarehouseFee WHERE feeID = p_feeID;
END $$
DELIMITER ;

-- 요금 ID 로 요금 조회
DROP PROCEDURE IF EXISTS sp_SelectFeeById;
DELIMITER $$
CREATE PROCEDURE sp_SelectFeeById(
    IN p_feeID INT
)
BEGIN
    SELECT * FROM WarehouseFee WHERE feeID = p_feeID;
END $$
DELIMITER ;

-- 창고 ID 로 요금 조회
DROP PROCEDURE IF EXISTS sp_SelectFeesByWarehouseId;
DELIMITER $$
CREATE PROCEDURE sp_SelectFeesByWarehouseId(
    IN p_warehouseID INT
)
BEGIN
    SELECT * FROM WarehouseFee WHERE warehouseID = p_warehouseID;
END $$
DELIMITER ;
