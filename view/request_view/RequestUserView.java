package view.request_view;

import vo.Requests.Request;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStreamReader;
import java.util.List;
import java.time.format.DateTimeFormatter;

public class RequestUserView {
    private final BufferedReader input = new BufferedReader(new InputStreamReader(System.in));
    private final DateTimeFormatter fmt = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");

    public int mainMenu() throws IOException {
        while (true) {
            System.out.println("\n--- 게시판 ---");
            System.out.println("1. 문의하기 | 2. 공지사항 | 3. 뒤로가기");
            System.out.print("[메뉴 선택]: ");
            try {
                int choice = Integer.parseInt(input.readLine().trim());
                if (choice >= 1 && choice <= 3) return choice;
                else System.out.println("1~3 숫자만 입력해주세요.");
            } catch (NumberFormatException e) { System.out.println("숫자를 입력해주세요."); }
        }
    }

    public int requestMenu() throws IOException {
        while (true) {
            System.out.println("\n--- 문의하기 ---");
            System.out.println("1. 문의 게시판 | 2. 1:1 문의 | 3. 뒤로가기");
            System.out.print("[메뉴 선택]: ");
            try {
                int choice = Integer.parseInt(input.readLine().trim());
                if (choice >= 1 && choice <= 3) return choice;
                else System.out.println("1~3 숫자만 입력해주세요.");
            } catch (NumberFormatException e) { System.out.println("숫자를 입력해주세요."); }
        }
    }

    public int requestBoardMenu() throws IOException {
        while (true) {
            System.out.println("\n--- 문의 게시판 메뉴 ---");
            System.out.println("1. 글쓰기 | 2. 내 글 수정 | 3. 전체 문의 조회 | 4. 내 문의 조회 | 5. 글 삭제 | 6. 뒤로가기");
            System.out.print("[메뉴 선택]: ");
            try {
                int choice = Integer.parseInt(input.readLine().trim());
                if (choice >= 1 && choice <= 6) return choice;
                else System.out.println("1~6 숫자만 입력해주세요.");
            } catch (NumberFormatException e) { System.out.println("숫자를 입력해주세요."); }
        }
    }

    public int oneToOneMenu() throws IOException {
        while (true) {
            System.out.println("\n--- 1:1 문의 메뉴 ---");
            System.out.println("1. 글쓰기 | 2. 내 글 수정 | 3. 내 문의 조회 | 4. 글 삭제 | 5. 뒤로가기");
            System.out.print("[메뉴 선택]: ");
            try {
                int choice = Integer.parseInt(input.readLine().trim());
                if (choice >= 1 && choice <= 5) return choice;
                else System.out.println("1~5 숫자만 입력해주세요.");
            } catch (NumberFormatException e) { System.out.println("잘못된 입력입니다."); }
        }
    }

    public Request createRequest() throws IOException {
        Request request = new Request();
        System.out.print("제목: "); request.setR_title(input.readLine().trim());
        System.out.print("내용: "); request.setR_content(input.readLine().trim());
        request.setR_status("대기");
        return request;
    }

    public int updateRequest() throws IOException {
        System.out.print("수정할 글의 ID를 입력해주세요: ");
        try { return Integer.parseInt(input.readLine().trim()); }
        catch (NumberFormatException e) { System.out.println("ID는 숫자로 입력해주세요."); return -1; }
    }

    public int deleteRequest() throws IOException {
        System.out.print("삭제할 글의 ID를 입력해주세요: ");
        int id;
        try { id = Integer.parseInt(input.readLine().trim()); }
        catch (NumberFormatException e) { System.out.println("ID는 숫자로 입력해주세요."); return -1; }

        System.out.print("정말 삭제하시겠습니까? (Y/N): ");
        if (!input.readLine().equalsIgnoreCase("Y")) { System.out.println("삭제가 취소되었습니다."); return -1; }
        return id;
    }

    public void selectAllRequest(List<Request> requests) {
        if (requests == null || requests.isEmpty()) { System.out.println("조회할 글이 없습니다."); return; }
        System.out.println("\n--- 전체 문의 목록 ---");
        System.out.println("ID\t제목\t유형\t작성일\t작성자ID");
        for (Request req : requests) {
            String type = req.getR_type() != null ? req.getR_type().name() : "없음";
            String date = req.getR_createAt() != null ? req.getR_createAt().format(fmt) : "없음";
            System.out.printf("%d\t%s\t%s\t%s\t%d\n",
                    req.getRequestID(), req.getR_title(), type, date, req.getUid());
        }
    }

    public void selectMyRequest(List<Request> requests) {
        if (requests == null || requests.isEmpty()) { System.out.println("조회할 글이 없습니다."); return; }
        System.out.println("\n--- 내 문의 목록 ---");
        System.out.println("ID\t제목\t유형\t작성일");
        for (Request req : requests) {
            String type = req.getR_type() != null ? req.getR_type().name() : "없음";
            String date = req.getR_createAt() != null ? req.getR_createAt().format(fmt) : "없음";
            System.out.printf("%d\t%s\t%s\t%s\n",
                    req.getRequestID(), req.getR_title(), type, date);
        }
    }
}
